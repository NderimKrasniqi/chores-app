...
type HouseholdSection =
  "overview" | "chores" | "reviews" | "payouts" | "access";
...
function SectionButton({
  ...
  return (
    <Pressable
      className={
        active
          ? "flex-1 rounded-xl bg-white px-2 py-3"
          : "flex-1 rounded-xl px-2 py-3"
      }
      onPress={onPress}
    >
      <Text
        className={
          active
            ? "text-center text-xs font-semibold text-slate-950"
...
export function HouseholdCard({ household }: HouseholdCardProps) {
  const [section, setSection] = useState<HouseholdSection>("overview");

  return (
    <View className="mt-5 rounded-2xl border border-slate-800 bg-slate-900 p-5">
      <Text className="text-2xl font-bold text-white">{household.name}</Text>
      ...
      <View className="mt-5 flex-row rounded-2xl bg-slate-950 p-1">
        ...
        <SectionButton
          active={section === "chores"}
          label="Chores"
          onPress={() => setSection("chores")}
        />
        ...
        <SectionButton
          ...
          label="Access"
          onPress={() => setSection("access")}
        />
      </View>

      <ParentHouseholdActivity
        householdId={household.householdId}
        showHistory={section === "overview"}
      />
      ...
      {section === "overview" && (
        <View className="mt-6">
          ...
          <View className="mt-6 border-t border-slate-800 pt-5">
            ...
            {household.children.length === 0 ? (
              ...
              household.children.map((child) => (
                <View
                  key={child.childId}
                  className="mt-3 rounded-xl bg-slate-800 px-4 py-3"
                >
          ...
          <ActiveClaimableClaimsCard householdId={household.householdId} />
        </View>
      )}
      ...
      {section === "reviews" && (
        <View className="mt-6">
          <PersonalChoreReviewsCard householdId={household.householdId} />

          <ClaimableChoreReviewsCard householdId={household.householdId} />

          <RedoChoreReviewsCard householdId={household.householdId} />
        </View>
      )}

      {section === "payouts" && (
        <View className="mt-6">
          <PayoutsCard householdId={household.householdId} />
        </View>
      )}

      {section === "access" && (
        <View className="mt-6">
          <Text className="font-semibold text-white">Child devices</Text>

          <Text className="mt-2 text-sm leading-5 text-slate-500">
            Pair devices, review active access, or revoke a Child device.
          </Text>
          ...
          {household.children.map((child) => (
            ...
            >
              ...
              <ChildDeviceList childId={child.childId} />
            </View>
          ))}
