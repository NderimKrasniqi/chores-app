...
export function Task15PayoutsMaestroFixtureScreen() {
  const [payoutWeekday, setPayoutWeekday] =
    useState<(typeof weekdays)[number]>("friday");
    ...
    >
      ...
      <View className="mt-5 rounded-2xl bg-slate-900 p-5">
        ...
        <View className="mt-2 flex-row flex-wrap">
          {weekdays.map((weekday) => (
            <Pressable
              ...
              onPress={() => setPayoutWeekday(weekday)}
              className={
        ...
        <View className="mt-5 rounded-xl bg-slate-800 p-4">
          <Text className="text-base font-semibold text-white">Alex</Text>
          ...
          {!paid ? (
            <View className="mt-4 border-t border-slate-700 pt-4">
              <Text className="font-semibold text-white">70 kr to pay</Text>

              <Text className="mt-1 text-sm leading-5 text-slate-400">
                Payout period ending 2030-01-18. Pay with Swish, then confirm
                below.
              </Text>

              <Text className="mt-2 text-sm leading-5 text-amber-300">
                1 unresolved chore outcome will be handled in a later payout.
              </Text>
        ...
        <View className="mt-5 rounded-xl bg-slate-800 p-4">
          <Text className="text-base font-semibold text-white">Sam</Text>
