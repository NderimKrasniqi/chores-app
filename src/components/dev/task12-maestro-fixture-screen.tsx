...
const CHILD_ID = "maestro-task12-child" as Id<"children">;

const GARAGE_OCCURRENCE_ID =
  "maestro-task12-garage-occurrence" as Id<"choreOccurrences">;

const GARAGE_CLAIM_ID = "maestro-task12-garage-claim" as Id<"choreClaims">;
...
type FixtureClaimState = "none" | "claimed" | "submitted";

export function Task12MaestroFixtureScreen() {
  const [claimState, setClaimState] = useState<FixtureClaimState>("none");
  ...
  if (claimState === "none") {
    claimableOccurrences.push({
      occurrenceId: GARAGE_OCCURRENCE_ID,
   ...
   */
  claimableOccurrences.push({
    occurrenceId: WINDOWS_OCCURRENCE_ID,

    title: "Clean windows",

    description: "Clean the downstairs windows.",
      ...
      : [
          {
            ...
            commitment: claimState === "claimed" ? commitment : null,
          },
        ];
  ...
  return (
    ...
    >
      ...
      <Text className="mt-2 text-sm leading-5 text-slate-400">
        Deterministic Claimable submission and pending-review journey.
      </Text>
      ...
      <ClaimableChoresView
        ...
        onClaim={async (occurrenceId, acceptImmediateLock) => {
          if (claimState !== "none") {
            throw new Error("Fixture Child already owns an active Claim.");
          }

          if (occurrenceId !== GARAGE_OCCURRENCE_ID) {
            throw new Error(
        ...
        onUnclaim={async (claimId) => {
          if (claimState !== "claimed" || claimId !== GARAGE_CLAIM_ID) {
            throw new Error("Unexpected TASK-12 unclaim.");
          }

          setClaimState("none");
        }}
        onSubmit={async (claimId) => {
          if (claimState !== "claimed" || claimId !== GARAGE_CLAIM_ID) {
            throw new Error(
