...
export function ClaimableChoreReviewsView({
  ...
  pending: ClaimableChoreReviewViewModel[];

  onApprove: (submissionId: Id<"choreSubmissions">) => Promise<{
    amountSek: number;
  }>;
  ...
  async function handleApprove(submission: ClaimableChoreReviewViewModel) {
    if (reviewingId !== null) {
      return;
    }
    ...
    try {
      const result = await onApprove(submission.submissionId);
  ...
  async function handleReject(
    submission: ClaimableChoreReviewViewModel,
    redoDeadlineLocalDate: string,
    redoDeadlineLocalTime: string,
  ) {
    if (reviewingId !== null || !onReject) {
      return;
    }
  ...
  return (
    <View className="mt-6 border-t border-slate-800 pt-6">
      ...
      <Text className="mt-2 text-sm leading-5 text-slate-500">
        Approve completed extra chores or reject the first submission with one
        Redo deadline. The active Claim remains occupied during Redo.
      </Text>
      ...
      {pending.length === 0 ? (
        <View className="mt-4 rounded-2xl bg-slate-950 p-5">
          ...
          <Text className="mt-2 text-sm leading-5 text-slate-500">
            Submitted Claimable Chores will appear here for Parent review.
          </Text>
        </View>
      ) : (
        <View className="mt-4">
            ...
            return (
              ...
              >
                ...
                <View className="mt-4 border-t border-slate-800 pt-3">
                  ...
                  <Text className="text-xs leading-5 text-slate-500">
                    Original deadline:{" "}
                    {formatDateTime(submission.deadlineAt, submission.timezone)}
                  </Text>

                  <Text className="text-xs leading-5 text-slate-500">
                    Household timezone: {submission.timezone}
                  </Text>
                </View>
                ...
                <Text className="mt-3 text-xs leading-5 text-slate-500">
                  Approval creates the {submission.valueSek} kr earning and
                  releases this Child&apos;s active Claim slot.
                </Text>
