...
export function RedoChoreReviewsView({
  ...
  pending: RedoChoreReviewViewModel[];

  onApprove: (submission: RedoChoreReviewViewModel) => Promise<{
    amountSek: number;
  }>;

  onReject: (submission: RedoChoreReviewViewModel) => Promise<unknown>;
}) {
  ...
  async function handleApprove(submission: RedoChoreReviewViewModel) {
    if (reviewingId !== null) {
      return;
    }
    ...
    setReviewingId(submission.submissionId);

    setReviewAction("approve");

    try {
      const result = await onApprove(submission);
  ...
  async function handleReject(submission: RedoChoreReviewViewModel) {
    if (reviewingId !== null) {
      return;
    }
  ...
  return (
    <View className="mt-6 border-t border-slate-800 pt-6">
      ...
      <Text className="mt-2 text-sm leading-5 text-slate-500">
        These are second and final submissions. Approve to complete the chore,
        or reject to end it as failed.
      </Text>

      {actionMessage ? (
        <View className="mt-4 rounded-xl border border-green-900 bg-green-950 p-3">
      ...
      {actionError ? (
        <View className="mt-4 rounded-xl border border-red-900 bg-red-950 p-3">
          <Text className="text-sm leading-5 text-red-300">{actionError}</Text>
        </View>
      ) : null}

      {pending.length === 0 ? (
        <View className="mt-4 rounded-2xl bg-slate-950 p-5">
          <Text className="font-semibold text-white">No Redos waiting</Text>

          <Text className="mt-2 text-sm leading-5 text-slate-500">
            Submitted Redos will appear here for their final Parent review.
          </Text>
        </View>
      ) : (
        <View className="mt-4">
            ...
            return (
              ...
              >
                <View className="flex-row items-start justify-between">
                  <View className="flex-1 pr-4">
                    <Text className="text-xs font-semibold uppercase tracking-wider text-amber-400">
                      {submission.kind === "personal"
                        ? "Personal Redo"
                        : "Claimable Redo"}
                    </Text>

                    <Text className="mt-1 text-xs font-semibold uppercase tracking-wider text-slate-500">
                ...
                <Pressable
                  ...
                  onPress={() => void handleApprove(submission)}
                  className={
                    actionsBusy
                ...
                >
                  ...
                  >
                    {isApproving ? "Approving Redo…" : "Approve Redo"}
                  </Text>
                </Pressable>
                ...
                <Pressable
                  ...
                  disabled={actionsBusy}
                  onPress={() => void handleReject(submission)}
                  className={
                    actionsBusy
                      ? "mt-3 items-center rounded-xl bg-slate-800 px-4 py-3"
                      : "mt-3 items-center rounded-xl bg-red-900 px-4 py-3"
                  }
                >
                  <Text
                    className={
                  ...
                  >
                    {isRejecting ? "Rejecting Redo…" : "Reject Redo"}
                  </Text>
                </Pressable>

                <Text className="mt-3 text-xs leading-5 text-slate-500">
                  A rejected Redo is final. There is no second correction
                  opportunity.
                </Text>
              </View>
            );
