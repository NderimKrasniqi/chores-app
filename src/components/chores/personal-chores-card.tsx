...
function formatState(state: OccurrenceState) {
  switch (state) {
    case "scheduled":
      return "Upcoming";
...
function ChoreCard({
  ...
  occurrence: PersonalChoreOccurrence;
  ...
  return (
    <View
      className={
        compact
          ? "mb-3 rounded-xl bg-slate-950 p-4"
          : "mb-3 rounded-xl border border-slate-800 bg-slate-950 p-4"
      }
    >
      <View className="flex-row items-start justify-between">
        <View className="flex-1 pr-3">
          ...
          {!compact && occurrence.description ? (
      ...
      {occurrence.isUnlockChore && !compact ? (
      ...
      <Text className="mt-3 text-sm font-medium text-slate-300">
        {formatState(occurrence.state)}
      </Text>

      <Text className="mt-1 text-xs leading-5 text-slate-500">
        Scheduled: {occurrence.scheduledLocalDate}
      </Text>
...
export function PersonalChoresCard() {
  ...
  const [submittingId, setSubmittingId] = useState<
    Id<"choreOccurrences"> | undefined
  >(undefined);

  const redoByOccurrence = useMemo(
    () => new Map((redos ?? []).map((redo) => [redo.occurrenceId, redo])),
    [redos],
  );
  ...
  const presentation = useMemo(() => {
    if (!occurrences || !redos) {
      return undefined;
    }

    const typedOccurrences = occurrences as PersonalChoreOccurrence[];

    const groups = new Map<Id<"choreDefinitions">, PersonalChoreOccurrence[]>();

    for (const occurrence of typedOccurrences) {
      const existing = groups.get(occurrence.choreDefinitionId);

      if (existing) {
        existing.push(occurrence);
      } else {
        groups.set(occurrence.choreDefinitionId, [occurrence]);
      }
    }

    const currentAndNext: PersonalChoreOccurrence[] = [];
    ...
    for (const group of groups.values()) {
      ...
      if (unresolved.length > 0) {
        currentAndNext.push(...unresolved);

        continue;
      }

      const nextUpcoming = ordered.find(
        (occurrence) => occurrence.state === "scheduled",
      );

      if (nextUpcoming) {
        currentAndNext.push(nextUpcoming);
      }
    }

    currentAndNext.sort((left, right) => {
      ...
      return left.availabilityStartsAt - right.availabilityStartsAt;
    });
  ...
  async function handleSubmit(
    ...
    try {
      ...
      Alert.alert("Submitted", `${title} was sent to your parent for review.`);
    } catch (error) {
      Alert.alert(
        "Could not submit",
  ...
  if (presentation === undefined) {
    return (
      <View className="mt-4 rounded-2xl bg-slate-900 p-5">
        <Text className="font-semibold text-white">Personal chores</Text>

        <Text className="mt-2 text-sm text-slate-500">Loading chores…</Text>
      </View>
    );
  }

  const { currentAndNext, recentHistory } = presentation;

  return (
    <View className="mt-4 rounded-2xl bg-slate-900 p-5">
      <Text className="text-lg font-semibold text-white">Personal chores</Text>

      <Text className="mt-1 text-sm leading-5 text-slate-500">
        What needs your attention now, plus your next upcoming responsibility.
      </Text>

      {currentAndNext.length === 0 ? (
        <View className="mt-4 rounded-xl bg-slate-950 p-4">
          <Text className="font-semibold text-white">Nothing coming up</Text>
        ...
        <View className="mt-4">
          {currentAndNext.map((occurrence) => {
            const redo = redoByOccurrence.get(occurrence.occurrenceId);

            if (occurrence.state === "redo_required" && redo) {
              return (
                <ChildRedoRequiredCard
                  ...
                  deadlineAt={redo.deadlineAt}
                  timezone={occurrence.timezone}
                  claimRemainsActive={false}
                  canSubmit={redo.canSubmitRedo}
                  submitting={submittingId === occurrence.occurrenceId}
      ...
      {recentHistory.length > 0 ? (
        <View className="mt-3 border-t border-slate-800 pt-5">
          <Text className="font-semibold text-white">Recent</Text>

          <Text className="mb-3 mt-1 text-xs text-slate-500">
            Latest completed or missed Personal Chores.
          </Text>
