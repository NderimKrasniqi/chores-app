...
import { scheduleClaimNotifications } from "./lib/notifications/orchestration";
import {
...
function getCommitmentStatus({
  ...
  commitmentLockReachedAt?: number;
  ...
  const isTimeLocked =
    commitmentLockReachedAt !== undefined || status.isTimeLocked;

  const isImmediatelyLocked = isTimeLocked || !status.hasUnclaimAllowance;
  ...
  return {
    ...
    hasUnclaimAllowance: status.hasUnclaimAllowance,

    remainingUnclaims: status.remainingUnclaims,

    canUnclaim: !isTimeLocked && status.hasUnclaimAllowance,

    isImmediatelyLocked,

    lockReason: isTimeLocked
...
export const listMine = query({
  ...
  returns: claimableChoreListValidator,
  ...
  handler: async (ctx) => {
    ...
    const [visible, claimed, usage] = await Promise.all([
      listVisibleClaimableOccurrencesForChild(
        ctx,
        household._id,
        child._id,
      ...
      listHouseholdClaimedOccurrences(ctx, household._id),

      getWeeklyUnclaimUsageForChild(ctx, household, child._id, now),
    ]);

    return {
      gate: {
        canAccessClaimables: visible.gate.canAccessClaimables,

        reason: visible.gate.reason,

        currentUnlockOccurrence: visible.gate.currentUnlockOccurrence
          ? {
              ...
              scheduledLocalDate:
                visible.gate.currentUnlockOccurrence.scheduledLocalDate,

              availabilityStartsAt:
                visible.gate.currentUnlockOccurrence.availabilityStartsAt,

              deadlineAt: visible.gate.currentUnlockOccurrence.deadlineAt,
            }
          : null,
      },
      ...
      unclaimAllowance: {
        ...
        remainingUnclaims: usage.remainingUnclaims,

        payoutWeek: {
          startLocalDate: usage.payoutWeek.startLocalDate,

          endLocalDate: usage.payoutWeek.endLocalDate,

          startAt: usage.payoutWeek.startAt,

          endAt: usage.payoutWeek.endAt,
        },
      },

      claimableOccurrences: visible.occurrences.map((occurrence) => ({
        ...
        commitment: getCommitmentStatus({
          ...
          commitmentLockReachedAt: occurrence.commitmentLockReachedAt,
      ...
      claimedOccurrences: claimed.map((item) => {
        const isMine = item.child._id === child._id;

        const commitment =
          isMine && item.claim.state === "claimed"
            ? getCommitmentStatus({
                deadlineAt: item.occurrence.deadlineAt,

                commitmentLockReachedAt:
                  item.occurrence.commitmentLockReachedAt,
        ...
        return {
          claimId: item.claim._id,

          occurrenceId: item.occurrence._id,

          childId: item.child._id,

          claimedByDisplayName: item.child.displayName,
          ...
          valueSek: item.occurrence.valueSek,

          scheduledLocalDate: item.occurrence.scheduledLocalDate,
...
export const listActiveForParent = query({
  args: {
    householdId: v.id("households"),
  },

  returns: activeClaimsForParentValidator,

  handler: async (ctx, args) => {
    await requireCurrentParentForHousehold(ctx, args.householdId);

    const claimed = await listHouseholdClaimedOccurrences(
      ctx,
      args.householdId,
    );
    ...
    return await Promise.all(
      claimed.map(async (item) => {
        const redo =
          item.claim.state === "redo_required"
            ? await ctx.db
                .query("choreRedos")
                .withIndex("by_occurrence", (q) =>
                  q.eq("occurrenceId", item.occurrence._id),
                )
                .unique()
            : null;
        ...
        return {
          ...
          claimState: item.claim.state as ActiveClaimState,
...
export const claim = mutation({
  args: {
    ...
    acceptImmediateLock: v.optional(v.boolean()),
  },
  ...
  handler: async (ctx, args) => {
    ...
    const result = await claimClaimableOccurrence(
      ...
      args.occurrenceId,
      now,
      args.acceptImmediateLock ?? false,
    );

    await scheduleClaimNotifications(ctx, result.claimId, {
      now,
    });
...
export const unclaim = mutation({
  ...
  returns: unclaimResultValidator,
