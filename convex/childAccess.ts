...
import { getUniqueActiveChildAccessGrantForAuthUser } from "./lib/childAccess/activeGrants";
 ...
 */
export const getLocalGrantStatuses = query({
  args: {
    accessGrantIds: v.array(v.id("childDeviceAccessGrants")),
  },
  ...
  handler: async (ctx, args) => {
    const uniqueIds = Array.from(new Set(args.accessGrantIds));

    const statuses: {
      accessGrantId: (typeof uniqueIds)[number];

      isActive: boolean;
    }[] = [];

    for (const accessGrantId of uniqueIds) {
      const grant = await ctx.db.get(accessGrantId);

      statuses.push({
        accessGrantId,

        isActive: grant !== null && grant.revokedAt === undefined,
      });
    }
