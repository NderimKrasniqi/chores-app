...
export const notificationEventKindValidator = v.union(
  v.literal("claimable_available"),
  v.literal("submission_review"),
  v.literal("approved"),
...
export const notificationTables = {
  pushRegistrations: defineTable({
    ...
    childAccessGrantId: v.optional(v.id("childDeviceAccessGrants")),
    ...
    .index("by_auth_user_and_expo_push_token", ["authUserId", "expoPushToken"])
    .index("by_auth_user", ["authUserId"])
    .index("by_child", ["childId"])
    .index("by_child_access_grant", ["childAccessGrantId"]),
  ...
  notificationEvents: defineTable({
    ...
    occurrenceId: v.optional(v.id("choreOccurrences")),

    submissionId: v.optional(v.id("choreSubmissions")),

    claimId: v.optional(v.id("choreClaims")),

    redoId: v.optional(v.id("choreRedos")),

    scheduledFor: v.number(),

    createdAt: v.number(),

    dispatchAttemptCount: v.number(),
  ...
  pushDeliveries: defineTable({
    ...
    .index("by_event_and_registration", ["eventId", "registrationId"])
