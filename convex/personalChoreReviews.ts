...
import {
  ...
  pendingPersonalReviewsValidator,
  personalApprovalResultValidator,
...
export const listPending = query({
  ...
  returns: pendingPersonalReviewsValidator,

  handler: async (ctx, args) => {
    await requireCurrentParentForHousehold(ctx, args.householdId);

    return await listPendingPersonalReviews(ctx, args.householdId);
  },
});
...
export const approve = mutation({
  ...
  returns: personalApprovalResultValidator,
  ...
  handler: async (ctx, args) => {
    ...
    const result = await approvePersonalSubmission(
      ctx,
      submission._id,
      authUser._id,
    );
