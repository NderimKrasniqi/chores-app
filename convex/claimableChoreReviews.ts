...
import { scheduleRedoDeadlineFailure } from "./lib/redos/deadlineFailure";
import {
...
export const listPending = query({
  ...
  returns: pendingClaimableReviewsValidator,

  handler: async (ctx, args) => {
    await requireCurrentParentForHousehold(ctx, args.householdId);

    return await listPendingClaimableReviews(ctx, args.householdId);
  },
});
...
export const approve = mutation({
  ...
  returns: claimableApprovalResultValidator,
  ...
  handler: async (ctx, args) => {
    ...
    const result = await approveClaimableSubmission(
      ctx,
      submission._id,
      authUser._id,
    );

    await notifyChildOfApproval(ctx, result.reviewId);
...
export const reject = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    await notifyChildRedoRequired(ctx, result.redoId);

    return result;
  },
});

export const approveRedo = mutation({
  args: {
    submissionId: v.id("choreSubmissions"),
  },

  returns: redoApprovalResultValidator,
  ...
  handler: async (ctx, args) => {
    ...
    const result = await approveRedoSubmission(
      ctx,
      submission._id,
      authUser._id,
      "claimable",
...
export const rejectRedo = mutation({
  args: {
    submissionId: v.id("choreSubmissions"),
  },

  returns: redoRejectionResultValidator,
