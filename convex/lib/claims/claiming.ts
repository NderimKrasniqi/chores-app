...
const activeClaimStates = ["claimed", "submitted", "redo_required"] as const;

async function findActiveClaimForChild(
  ctx: MutationCtx,
  childId: Id<"children">,
) {
  for (const state of activeClaimStates) {
...
export async function claimClaimableOccurrence(
  ...
  if (occurrence.kind !== "claimable") {
    throw new ConvexError("Only Claimable Chores can be claimed.");
  }

  if (occurrence.state !== "available") {
    throw new ConvexError("This Claimable Chore is not available.");
  }

  if (now < occurrence.availabilityStartsAt) {
    throw new ConvexError("This Claimable Chore is not available yet.");
  }

  if (now >= occurrence.deadlineAt) {
    throw new ConvexError("This Claimable Chore deadline has passed.");
  }
  ...
  if (activeClaim) {
    throw new ConvexError("This Child already has an active Claimable Chore.");
  }
  ...
  const unclaimStatus = getClaimUnclaimStatus({
  ...
  const isImmediatelyLocked =
    unclaimStatus.isTimeLocked || !unclaimStatus.hasUnclaimAllowance;

  if (isImmediatelyLocked && !acceptImmediateLock) {
    throw new ConvexError(
  ...
  return {
    ...
    commitment: {
      lockAt: unclaimStatus.lockAt,

      isImmediatelyLocked,

      remainingUnclaims: usage.remainingUnclaims,

      lockReason: unclaimStatus.isTimeLocked
        ? ("time_window" as const)
        : !unclaimStatus.hasUnclaimAllowance
