...
import { resolveRedoDeadline } from "../redos/deadline";
...
export async function rejectInitialSubmission(
  ...
  if (submission.attemptNumber !== 1) {
    throw new ConvexError("Only an initial Submission can create a Redo.");
  }
  ...
  if (occurrence.state !== "submitted") {
    throw new ConvexError("This Chore is not awaiting initial review.");
  }
   ...
   */
  if (submission.submittedAt > occurrence.deadlineAt) {
    throw new ConvexError("A late initial Submission cannot receive a Redo.");
  }
  ...
  const existingRedo = await ctx.db
    .query("choreRedos")
   ...
   */
  const redoDeadline = resolveRedoDeadline({
    deadlineLocalDate: redoDeadlineLocalDate,

    deadlineLocalTime: redoDeadlineLocalTime,
  ...
  const redoId = await ctx.db.insert("choreRedos", {
    ...
    initialSubmissionId: submission._id,

    rejectionReviewId: reviewId,

    deadlineLocalDate: redoDeadline.deadlineLocalDate,

    deadlineLocalTime: redoDeadline.deadlineLocalTime,
  ...
  return {
    ...
    redoDeadlineAt: redoDeadline.deadlineAt,
