...
import { insertFinancialLedgerEntry } from "../finance/financialProjection";
...
async function findSubmittedClaim(
  ...
  return (
    claims.find(
      (claim) => claim.childId === childId && claim.state === "submitted",
    ) ?? null
  );
}
...
async function loadRedoReviewContext(
  ...
  if (occurrence.householdId !== submission.householdId) {
    throw new ConvexError(
  ...
  if (occurrence.state !== "submitted") {
    throw new ConvexError("This Redo is not awaiting Parent review.");
  }
  ...
  if (redo.householdId !== occurrence.householdId) {
    throw new ConvexError(
   ...
   * was on time.
   */
  if (submission.submittedAt > redo.deadlineAt) {
    throw new ConvexError("Late Redo Submissions cannot be reviewed.");
  }
  ...
  if (
    ...
    initialSubmission.childId !== submission.childId ||
    initialSubmission.attemptNumber !== 1
  ) {
    throw new ConvexError("Redo initial Submission history is invalid.");
  }

  const rejectionReview = await ctx.db.get(redo.rejectionReviewId);

  if (
    !rejectionReview ||
    rejectionReview.householdId !== occurrence.householdId ||
    rejectionReview.occurrenceId !== occurrence._id ||
    rejectionReview.submissionId !== initialSubmission._id ||
    rejectionReview.decision !== "rejected"
  ) {
    throw new ConvexError("Redo rejection Review history is invalid.");
  }
  ...
  let claim: Awaited<ReturnType<typeof findSubmittedClaim>> = null;

  if (occurrence.kind === "personal") {
    if (occurrence.personalChildId !== submission.childId) {
      throw new ConvexError(
        "Submission Child does not match the Personal Chore assignment.",
      );
    }
  } else {
    claim = await findSubmittedClaim(ctx, occurrence._id, submission.childId);

    if (!claim) {
      throw new ConvexError("Submitted Claim ownership could not be verified.");
    }
...
export async function approveRedoSubmission(
  ...
  const { submission, occurrence, redo, claim } = await loadRedoReviewContext(
    ctx,
    submissionId,
    expectedKind,
  );
  ...
  if (existingEarnings.length > 0) {
    throw new ConvexError("This Chore Occurrence already has an earning.");
  }
  ...
  const ledgerEntryId = await insertFinancialLedgerEntry(ctx, {
    householdId: occurrence.householdId,
...
export async function rejectRedoSubmission(
  ...
  if (claim) {
    await ensureClaimableFailurePenalty(ctx, claim._id, now);
  }
