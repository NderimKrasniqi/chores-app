...
import { insertFinancialLedgerEntry } from "./financialProjection";
...
export async function ensureClaimableFailurePenalty(
  ...
  if (!Number.isFinite(now)) {
    throw new ConvexError("Penalty timestamp must be finite.");
  }
  ...
  if (claim.state !== "failed") {
    throw new ConvexError("Only a failed Claim can receive a failure penalty.");
  }
  ...
  if (occurrence.householdId !== claim.householdId) {
  ...
  if (occurrence.kind !== "claimable") {
    throw new ConvexError("Personal Chores cannot receive a failure penalty.");
  }
  ...
  if (!Number.isSafeInteger(occurrence.valueSek) || occurrence.valueSek <= 0) {
    throw new ConvexError(
  ...
  if (!child) {
    throw new ConvexError("Claim Child not found.");
  }

  if (
    child.householdId !== claim.householdId ||
    child.householdId !== occurrence.householdId
  ) {
    throw new ConvexError(
  ...
  const penalties = await ctx.db
    .query("ledgerEntries")
    .withIndex("by_occurrence_kind", (q) =>
      q.eq("occurrenceId", occurrence._id).eq("kind", "penalty"),
    )
    .collect();
  ...
  const expectedAmountSek = -occurrence.valueSek;

  const existingPenalty = penalties[0];
  ...
  if (existingPenalty) {
    if (
      existingPenalty.householdId !== occurrence.householdId ||
      existingPenalty.childId !== claim.childId ||
      existingPenalty.amountSek !== expectedAmountSek
    ) {
      throw new ConvexError(
    ...
    return {
      ledgerEntryId: existingPenalty._id,
      ...
      amountSek: existingPenalty.amountSek,

      createdAt: existingPenalty.createdAt,
