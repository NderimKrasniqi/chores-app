import { resolveLocalDateTimeToEpochMs } from "../scheduling/choreScheduling";
...
export function resolveRedoDeadline(
  input: ResolveRedoDeadlineInput,
): ResolvedRedoDeadline {
  if (!Number.isFinite(input.reviewedAt)) {
    throw new Error("Review time must be a finite timestamp.");
  }

  const deadlineAt = resolveLocalDateTimeToEpochMs(
    input.deadlineLocalDate,
  ...
  if (deadlineAt <= input.reviewedAt) {
    throw new Error("Redo deadline must be after the Parent review decision.");
  }
