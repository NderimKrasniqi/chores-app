...
export async function listActiveRedosForChild(
  ...
  const personal = await ctx.db
    ...
    .withIndex("by_personal_child_state_availability", (q) =>
      q.eq("personalChildId", childId).eq("state", "redo_required"),
    )
    .collect();
  ...
  const claims = await ctx.db
    ...
    .withIndex("by_child_state", (q) =>
      q.eq("childId", childId).eq("state", "redo_required"),
    )
    .collect();

  const occurrenceIds = new Set<Id<"choreOccurrences">>();

  for (const occurrence of personal) {
    if (
      occurrence.householdId === householdId &&
      occurrence.kind === "personal"
    ) {
      occurrenceIds.add(occurrence._id);
    }
  }

  for (const claim of claims) {
    if (claim.householdId === householdId) {
      occurrenceIds.add(claim.occurrenceId);
    }
  }
  ...
  for (const occurrenceId of occurrenceIds) {
    ...
    if (!initialSubmission || initialSubmission.childId !== childId) {
      continue;
    }
    ...
    result.push({
      ...
      deadlineLocalDate: redo.deadlineLocalDate,

      deadlineLocalTime: redo.deadlineLocalTime,
  ...
  return result.sort((left, right) => left.deadlineAt - right.deadlineAt);
}
