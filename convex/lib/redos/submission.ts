...
async function loadRedoForSubmission(
  ...
  if (occurrence.state !== "redo_required") {
    throw new ConvexError("This Chore is not awaiting a Redo submission.");
  }
  ...
  if (now < redo.createdAt) {
    throw new ConvexError("This Redo opportunity is not available yet.");
  }
  ...
  if (
    ...
    initialSubmission.childId !== childId ||
  ...
  const existingRedoSubmission = await ctx.db
  ...
  if (existingRedoSubmission) {
    throw new ConvexError("This Redo has already been submitted.");
  }
...
export async function submitPersonalRedo(
  ...
  const { occurrence, redo } = await loadRedoForSubmission(
    ctx,
    occurrenceId,
    childId,
    "personal",
...
export async function submitClaimableRedo(
  ...
  if (claim.state !== "redo_required") {
    throw new ConvexError("This Claim is not awaiting a Redo submission.");
  }
