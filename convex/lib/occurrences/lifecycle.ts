...
async function getClaimableDeadlineOwnership(
  ...
  const claims = await ctx.db
    .query("choreClaims")
    .withIndex("by_occurrence", (query) =>
      query.eq("occurrenceId", occurrenceId),
    )
    .collect();
   ...
   */
  const activeUnsubmittedClaims = claims.filter(
    (claim) => claim.state === "claimed",
  );

  if (activeUnsubmittedClaims.length > 1) {
    throw new ConvexError(
      "Claimable Chore has multiple active unsubmitted Claims.",
    );
  }

  const activeUnsubmittedClaim = activeUnsubmittedClaims[0] ?? null;
   ...
   */
  const protectedFromUnclaimedExpiry = claims.some(
...
export async function reconcileOccurrenceLifecycle(
   ...
   * lifecycle instead.
   */
  if (previousState !== "scheduled" && previousState !== "available") {
    return {
      found: true,
  ...
  let nextState: Doc<"choreOccurrences">["state"] = previousState;

  if (nextState === "scheduled" && now >= occurrence.availabilityStartsAt) {
    nextState = "available";
  }
  ...
  if (
    ...
    const { activeUnsubmittedClaim, protectedFromUnclaimedExpiry } =
      await getClaimableDeadlineOwnership(ctx, occurrenceId);
     ...
     */
    if (activeUnsubmittedClaim && now > occurrence.deadlineAt) {
      if (activeUnsubmittedClaim.householdId !== occurrence.householdId) {
        throw new ConvexError(
          "Claim Household does not match its Chore Occurrence.",
        );
      }

      await ctx.db.patch(activeUnsubmittedClaim._id, {
        state: "failed",
      });
      ...
      await ensureClaimableFailurePenalty(ctx, activeUnsubmittedClaim._id, now);
     ...
     */
    if (!protectedFromUnclaimedExpiry) {
      nextState = "expired_unclaimed";
    }
  }
  ...
  const shouldMarkAvailabilityReached =
    occurrence.availabilityReachedAt === undefined &&
    now >= occurrence.availabilityStartsAt;

  if (!stateChanged && !shouldMarkAvailabilityReached) {
    return {
      found: true,
  ...
  await ctx.db.patch(occurrenceId, {
    ...
    ...(shouldMarkAvailabilityReached
      ? {
          availabilityReachedAt: occurrence.availabilityStartsAt,
        }
      : {}),
  });
