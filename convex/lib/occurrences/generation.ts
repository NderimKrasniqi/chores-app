...
import { scheduleOccurrenceNotifications } from "../notifications/orchestration";
...
function localDateDayNumber(value: string, label: string) {
...
function validateGenerationWindow(
  ...
  const fromDay = localDateDayNumber(fromLocalDate, "Generation start date");

  const throughDay = localDateDayNumber(
    throughLocalDate,
    "Generation end date",
  );
...
function determineInitialState(
  kind: Doc<"choreDefinitions">["kind"],
  availabilityStartsAt: number,
  deadlineAt: number,
  now: number,
): Doc<"choreOccurrences">["state"] {
  if (now < availabilityStartsAt) {
    return "scheduled";
  }

  if (kind === "claimable" && now >= deadlineAt) {
   ...
   */
  if (kind === "personal" && now > deadlineAt) {
    return "missed";
  }
...
export async function generateOccurrencesForWindow(
  ...
  validateGenerationWindow(fromLocalDate, throughLocalDate);
  ...
  for (const definition of definitions) {
    ...
    for (const scheduledLocalDate of scheduledDates) {
      ...
      if (definition.kind === "personal") {
        if (!definition.personalChildId) {
          throw new ConvexError(
      ...
      const commitmentLockAt =
        definition.kind === "claimable"
          ? getClaimCommitmentLockAt(schedule.deadlineAt)
          : null;
      ...
      const occurrenceId = await ctx.db.insert("choreOccurrences", {
        ...
        ...(definition.availabilityLocalTime !== undefined
          ? {
              availabilityLocalTime: definition.availabilityLocalTime,
            }
          : {}),
        ...
        availabilityStartsAt: schedule.availabilityStartsAt,

        ...(state !== "scheduled"
          ? {
              availabilityReachedAt: schedule.availabilityStartsAt,
            }
          : {}),

        deadlineAt: schedule.deadlineAt,

        ...(commitmentLockAt !== null && commitmentLockAt <= now
          ? {
              commitmentLockReachedAt: commitmentLockAt,
            }
          : {}),

        ...(personalChildId !== undefined
          ? {
              personalChildId,
            }
          : {}),

        ...(eligibleChildIds !== undefined
          ? {
              eligibleChildIds,
            }
          : {}),
      ...
      if (scheduleTransitions) {
        await scheduleOccurrenceNotifications(ctx, occurrenceId, {
          now,
        });
      }
       ...
       */
      if (scheduleTransitions && state === "scheduled") {
        await ctx.scheduler.runAt(
          schedule.availabilityStartsAt,

          internal.jobs.occurrences.transitions.reconcile,
      ...
      if (
        scheduleTransitions &&
        definition.kind === "claimable" &&
        commitmentLockAt !== null &&
        commitmentLockAt > now
      ) {
        await ctx.scheduler.runAt(
          commitmentLockAt,

          internal.jobs.claims.transitions.reconcileCommitmentLock,
