...
export async function listDueScheduledOccurrences(
  ctx: MutationCtx,
  now: number,
  householdIds: Id<"households">[] | undefined,
) {
  if (householdIds === undefined) {
    return await ctx.db
      .query("choreOccurrences")
      .withIndex("by_state_availability", (q) =>
        q.eq("state", "scheduled").lte("availabilityStartsAt", now),
      )
      .collect();
  }
...
export async function listDueAvailableOccurrences(
  ...
  if (householdIds === undefined) {
    return await ctx.db
      .query("choreOccurrences")
      .withIndex("by_state_deadline", (q) =>
        q.eq("state", "available").lte("deadlineAt", now),
      )
      .collect();
  }
