...
export const run = internalMutation({
  ...
  handler: async (ctx): Promise<SmokeTestResult[]> => {
    ...
    const definitionId = await ctx.db.insert("choreDefinitions", {
      ...
      eligibleChildIds: [childOneId, childTwoId],

      isUnlockChore: false,

      createdByAuthUserId: "task10-visibility-smoke",
     ...
     */
    const beforeChildOne = await listVisibleClaimableOccurrencesForChild(
      ctx,
      householdId,
      childOneId,
      now,
    ...
    const beforeChildTwo = await listVisibleClaimableOccurrencesForChild(
      ctx,
      householdId,
      childTwoId,
      now,
    ...
    results.push(
      result(
        ...
        beforeChildOne.occurrences.length === 1 &&
          beforeChildTwo.occurrences.length === 1,
      ),
    );
     ...
     * claiming Child.
     */
    const afterChildOne = await listVisibleClaimableOccurrencesForChild(
      ctx,
      householdId,
      childOneId,
      now,
    ...
    results.push(
      result(
        ...
        afterChildOne.occurrences.length === 0,
      ),
    );
     ...
     */
    const afterChildTwo = await listVisibleClaimableOccurrencesForChild(
      ctx,
      householdId,
      childTwoId,
      now,
    ...
    results.push(
      result(
        ...
        afterChildTwo.occurrences.length === 0,
      ),
    );
     ...
     * remains available.
     */
    const claimed = await listHouseholdClaimedOccurrences(ctx, householdId);

    results.push(
      result(
        "Household can see who claimed the occurrence",
        claimed.length === 1 &&
          claimed[0]?.claim._id === claim.claimId &&
          claimed[0]?.child._id === childOneId &&
          claimed[0]?.child.displayName === "Child One",
     ...
     */
    await ctx.db.patch(claim.claimId, {
      state: "submitted",
    });

    const submittedVisible = await listHouseholdClaimedOccurrences(
      ctx,
      householdId,
    );
    ...
    results.push(
      result(
        ...
        submittedVisible.length === 1 &&
          submittedVisible[0]?.claim.state === "submitted",
      ),
    );
    ...
    const approvedVisible = await listHouseholdClaimedOccurrences(
      ctx,
      householdId,
    );
     ...
     * Cleanup.
     */
    await ctx.db.delete(claim.claimId);
