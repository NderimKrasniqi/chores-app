...
export const run = internalMutation({
  ...
  handler: async (ctx) => {
    ...
    const householdId = await ctx.db.insert("households", {
      name: "TASK13 Personal review isolation smoke",
    ...
    const childId = await ctx.db.insert("children", {
      ...
      displayName: "Review Isolation Child",
    ...
    const redo = await submitPersonalRedo(
      ctx,
      occurrenceId,
      childId,
      redoSubmittedAt,
    ...
    const pending = await listPendingPersonalReviews(ctx, householdId);

    assert(
      !pending.some((review) => review.submissionId === redo.submissionId),
    ...
    return {
      ...
      redoSubmissionId: redo.submissionId,
    };
  },
});
