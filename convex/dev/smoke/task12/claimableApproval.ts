...
import { listPendingClaimableReviews } from "../../../lib/reviews/pendingClaimable";
...
export const run = internalMutation({
  ...
  handler: async (ctx) => {
    ...
    const reviewAfterDeadline = resolveLocalDateTimeToEpochMs(
      "2030-01-16",
      "20:00",
      "Europe/Stockholm",
    );
    ...
    const definitionId = await ctx.db.insert("choreDefinitions", {
      ...
      createdByAuthUserId: "task12-smoke",
    ...
    const ledgerEntryIds: Array<Id<"ledgerEntries">> = [];
    ...
    try {
      ...
      const valid = await createSubmittedFixture({
        title: "Approve me",
      });

      const initialPending = await listPendingClaimableReviews(
        ctx,
        householdId,
      );

      assert(
        initialPending.length === 1 &&
          initialPending[0]?.submissionId === valid.submissionId &&
          initialPending[0]?.claimId === valid.claimId,
      ...
      const approval = await approveClaimableSubmission(
        ctx,
        valid.submissionId,
        "task12-parent",
        reviewAfterDeadline,
      ...
      reviewIds.push(approval.reviewId);

      ledgerEntryIds.push(approval.ledgerEntryId);

      const review = await ctx.db.get(approval.reviewId);

      const earning = await ctx.db.get(approval.ledgerEntryId);

      assert(
        review?.decision === "approved" &&
          review.reviewedByAuthUserId === "task12-parent" &&
          review.reviewedAt === reviewAfterDeadline,
        "Approval must persist one authoritative Parent review.",
      );

      assert(
        earning?.kind === "earning" &&
          earning.amountSek === 125 &&
          earning.childId === childA,
      ...
      const approvedClaim = await ctx.db.get(valid.claimId);

      const approvedOccurrence = await ctx.db.get(valid.occurrenceId);
      ...
      console.log("✅ 3/8 approval completes both Claim and occurrence");
      ...
      assert(
        !activeAfterApproval.some((item) => item.claim._id === valid.claimId),
        "Approved Claim must release the Child active-Claim slot.",
      );

      passed += 1;

      console.log("✅ 4/8 approval releases active-Claim ownership");

      const pendingAfterApproval = await listPendingClaimableReviews(
        ctx,
        householdId,
      );
      ...
      assert(
        !pendingAfterApproval.some(
          (item) => item.submissionId === valid.submissionId,
        ),
      ...
      console.log("✅ 5/8 approved submission leaves pending-review queue");
      ...
      const reviewsForValid = await ctx.db
        .query("choreReviews")
        .withIndex("by_submission", (q) =>
          q.eq("submissionId", valid.submissionId),
        )
        .collect();

      const earningsForValid = await ctx.db
        .query("ledgerEntries")
        .withIndex("by_occurrence_kind", (q) =>
          q.eq("occurrenceId", valid.occurrenceId).eq("kind", "earning"),
        )
        .collect();

      assert(
        reviewsForValid.length === 1 && earningsForValid.length === 1,
      ...
      const late = await createSubmittedFixture({
        title: "Invalid late submission",

        submissionTime: deadlineAt + 1,
      });

      const pendingWithLate = await listPendingClaimableReviews(
        ctx,
        householdId,
      );
      ...
      assert(
        !pendingWithLate.some(
          (item) => item.submissionId === late.submissionId,
        ),
      ...
      console.log("✅ 7/8 late persisted submission cannot be approved");

      const mismatched = await createSubmittedFixture({
        title: "Mismatched ownership",

        claimChildId: childA,

        submissionChildId: childB,
      });

      const pendingWithMismatch = await listPendingClaimableReviews(
        ctx,
        householdId,
      );
      ...
      assert(
        !pendingWithMismatch.some(
          (item) => item.submissionId === mismatched.submissionId,
        ),
      ...
      console.log("✅ 8/8 Claim ownership mismatch is rejected");
      ...
      for (const ledgerEntryId of ledgerEntryIds) {
        ...
        if (entry) {
          await ctx.db.delete(ledgerEntryId);
        }
      }
