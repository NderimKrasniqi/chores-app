...
export const run = internalQuery({
  ...
  handler: async (): Promise<SmokeTestResult[]> => {
     ...
     */
    {
      const actual = getScheduledLocalDates(
        {
          kind: "one_off",
     ...
     */
    {
      ...
      const expected = ["2030-01-01", "2030-01-03", "2030-01-05", "2030-01-07"];
     ...
     */
    {
      ...
      const expected = ["2026-09-07", "2026-09-10", "2026-09-21", "2026-09-24"];
     ...
     */
    {
      ...
      const expected = ["2026-01-31", "2026-03-31"];
     ...
     */
    {
      ...
      const availability = iso(schedule.availabilityStartsAt);

      const deadline = iso(schedule.deadlineAt);

      const passed =
        availability === "2026-01-15T07:00:00.000Z" &&
     ...
     */
    {
      ...
      const passed =
        availability === "2026-07-15T06:00:00.000Z" &&
     ...
     */
    {
      ...
      const passed =
        availability === "2026-09-06T22:00:00.000Z" &&
        deadline === "2026-09-08T05:30:00.000Z";
     ...
     */
    {
      const stockholm = resolveOccurrenceSchedule({
        scheduledLocalDate: "2026-01-15",
      ...
      const newYork = resolveOccurrenceSchedule({
        scheduledLocalDate: "2026-01-15",
      ...
      results.push(
        result(
          ...
          stockholm.availabilityStartsAt !== newYork.availabilityStartsAt,

          `${iso(stockholm.availabilityStartsAt)} / ${iso(
            newYork.availabilityStartsAt,
          )}`,
        ),
      );
    }
     ...
     */
    {
      const schedule = resolveOccurrenceSchedule({
        ...
        availabilityLocalTime: "01:30",

        deadlineLocalTime: "03:30",

        deadlineDayOffset: 0,
      });

      const elapsed = schedule.deadlineAt - schedule.availabilityStartsAt;
      ...
      results.push(
        result(
          ...
          `${iso(schedule.availabilityStartsAt)} / ${iso(schedule.deadlineAt)}`,
        ),
      );
    }
