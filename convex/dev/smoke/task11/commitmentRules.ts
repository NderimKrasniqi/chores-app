...
export const run = internalQuery({
  ...
  handler: async () => {
    ...
    assert(
      lockAt === deadlineAt - 2 * 60 * 60 * 1000,
      "Lock boundary must be exactly two hours before deadline.",
    );

    passed += 1;

    console.log("✅ 1/10 lock boundary is exactly two hours before deadline");

    assert(
      !isClaimTimeLocked(deadlineAt, lockAt - 1),
      "One millisecond before lock boundary must remain unlocked.",
    );

    passed += 1;

    console.log("✅ 2/10 instant before boundary remains unlocked");

    assert(
      isClaimTimeLocked(deadlineAt, lockAt),
      "Exact lock boundary must be locked.",
    );

    passed += 1;

    console.log("✅ 3/10 exact boundary is locked");

    assert(
      isClaimTimeLocked(deadlineAt, lockAt + 30 * 60 * 1000),
      "Inside the two-hour window must remain locked.",
    );

    passed += 1;

    console.log("✅ 4/10 inside lock window remains locked");

    const allowedStatus = getClaimUnclaimStatus({
      deadlineAt,

      now: lockAt - 60 * 60 * 1000,
    ...
    assert(
      allowedStatus.remainingUnclaims === 1,
      "Exactly one weekly unclaim should remain.",
    );

    passed += 1;

    console.log("✅ 5/10 time plus remaining allowance permits unclaim");

    const exhaustedStatus = getClaimUnclaimStatus({
      deadlineAt,
    ...
    assert(
      exhaustedStatus.remainingUnclaims === 0,
    ...
    const overusedStatus = getClaimUnclaimStatus({
      deadlineAt,
    ...
    assert(
      overusedStatus.remainingUnclaims === 0,
      "Remaining allowance must never become negative.",
    );

    passed += 1;

    console.log("✅ 7/10 remaining allowance never becomes negative");
     ...
     * 2030-01-18.
     */
    const stockholmMidweek = resolveLocalDateTimeToEpochMs(
      "2030-01-16",
      "12:00",
      "Europe/Stockholm",
    );

    const fridayWindow = getCurrentPayoutWeekWindow({
      now: stockholmMidweek,
    ...
    assert(
      fridayWindow.startLocalDate === "2030-01-11",
      "Friday payout week should start on the preceding Friday.",
    );

    assert(
      fridayWindow.endLocalDate === "2030-01-18",
    ...
    const exactNextFriday = resolveLocalDateTimeToEpochMs(
      "2030-01-18",
      "00:00",
      "Europe/Stockholm",
    );

    const nextFridayWindow = getCurrentPayoutWeekWindow({
      now: exactNextFriday,
    ...
    assert(
      nextFridayWindow.startLocalDate === "2030-01-18",
      "Exact payout boundary must begin the new Payout Week.",
    );

    passed += 1;

    console.log("✅ 9/10 exact payout boundary starts the next week");
     ...
     */
    const dstStart = resolveLocalDateTimeToEpochMs(
      "2030-03-31",
      "00:00",
      "Europe/Stockholm",
    );

    const dstWindow = getCurrentPayoutWeekWindow({
      now: dstStart,
    ...
    const actualHours =
      (dstWindow.endAt - dstWindow.startAt) / (60 * 60 * 1000);
    ...
    console.log("✅ 10/10 payout week remains timezone-stable across DST");
