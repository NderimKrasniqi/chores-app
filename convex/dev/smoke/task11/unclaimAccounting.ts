...
export const run = internalMutation({
  ...
  handler: async (ctx) => {
    ...
    async function createClaim({
      ...
      state: "claimed" | "unclaimed" | "cancelled";
      ...
      const claimId = await ctx.db.insert("choreClaims", {
        ...
        cancelledByAuthUserId:
          cancelledAt !== undefined ? "task11-parent" : undefined,
      });
    ...
    try {
      ...
      const initial = await getWeeklyUnclaimUsageForChild(
        ctx,
        household,
        childId,
        now,
      ...
      assert(
        initial.usedUnclaims === 0,
        "Fresh week should have zero used unclaims.",
      );

      assert(
        initial.remainingUnclaims === 2,
        "Fresh week should retain full allowance.",
      );

      passed += 1;

      console.log("✅ 1/8 fresh payout week has full allowance");
      ...
      const afterPreviousWeek = await getWeeklyUnclaimUsageForChild(
        ctx,
        household,
        childId,
        now,
      ...
      assert(
        afterPreviousWeek.usedUnclaims === 0,
        "Previous payout-week unclaim must not count.",
      );

      passed += 1;

      console.log("✅ 2/8 previous payout-week unclaim is excluded");
      ...
      const afterOne = await getWeeklyUnclaimUsageForChild(
        ctx,
        household,
        childId,
        now,
      ...
      assert(
        afterOne.usedUnclaims === 1 && afterOne.remainingUnclaims === 1,
        "One successful current-week unclaim should consume one allowance.",
      );

      passed += 1;

      console.log("✅ 3/8 successful Child unclaim consumes one allowance");
      ...
      await createClaim({
        ...
        cancelledAt: resolveLocalDateTimeToEpochMs(
          "2030-01-13",
          "11:00",
          "Europe/Stockholm",
        ),
      ...
      const afterCancellation = await getWeeklyUnclaimUsageForChild(
        ctx,
        household,
        childId,
        now,
      ...
      assert(
        afterCancellation.usedUnclaims === 1,
        "Parent cancellation must not consume Child allowance.",
      );

      passed += 1;

      console.log("✅ 4/8 Parent cancellation does not consume allowance");
      ...
      const afterActive = await getWeeklyUnclaimUsageForChild(
        ctx,
        household,
        childId,
        now,
      ...
      assert(
        afterActive.usedUnclaims === 1,
        "Active Claim must not consume unclaim allowance.",
      );

      passed += 1;

      console.log("✅ 5/8 active Claim does not consume allowance");
      ...
      const exhausted = await getWeeklyUnclaimUsageForChild(
        ctx,
        household,
        childId,
        now,
      ...
      console.log("✅ 6/8 weekly allowance exhausts at configured limit");

      const nextWeek = resolveLocalDateTimeToEpochMs(
        "2030-01-18",
        "00:00",
        "Europe/Stockholm",
      );

      const reset = await getWeeklyUnclaimUsageForChild(
        ctx,
        household,
        childId,
        nextWeek,
      ...
      assert(
        reset.usedUnclaims === 0,
        "New payout week should not inherit prior usage.",
      );

      assert(
        reset.remainingUnclaims === 2,
        "New payout week should restore full allowance.",
      );

      passed += 1;

      console.log("✅ 7/8 exact payout boundary resets weekly usage");

      assert(
        exhausted.payoutWeek.startLocalDate === "2030-01-11" &&
          exhausted.payoutWeek.endLocalDate === "2030-01-18",
