...
export const run = internalMutation({
  ...
  handler: async (ctx) => {
    ...
    const definitionIds: Id<"choreDefinitions">[] = [];

    const occurrenceIds: Id<"choreOccurrences">[] = [];

    const submissionIds: Id<"choreSubmissions">[] = [];

    const reviewIds: Id<"choreReviews">[] = [];

    const ledgerIds: Id<"ledgerEntries">[] = [];
    ...
    async function createReviewedPersonal({
      ...
      const definitionId = await ctx.db.insert("choreDefinitions", {
        ...
        createdByAuthUserId: "task17-parent",
      ...
      const reviewId = await ctx.db.insert("choreReviews", {
        ...
        reviewedByAuthUserId: "task17-parent",
    ...
    const first = await createReviewedPersonal({
      childId: childA,
    ...
    const second = await createReviewedPersonal({
      childId: childB,
    ...
    const rejected = await createReviewedPersonal({
      childId: childB,

      title: "Rejected private outcome",
     ...
     */
    const penaltyId = await ctx.db.insert("ledgerEntries", {
      householdId,

      childId: childB,

      occurrenceId: rejected.occurrenceId,
     ...
     */
    await ctx.db.patch(second.definitionId, {
      title: "Renamed future definition",

      valueSek: 999,
    });

    const activity = await listHouseholdApprovalActivity(ctx, householdId);
