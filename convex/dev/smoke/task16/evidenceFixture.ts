...
export const run = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    const definitionId = await ctx.db.insert("choreDefinitions", {
      ...
      createdByAuthUserId: "task16-parent",
    ...
    assert(registration.accepted, "Valid JPEG evidence was not accepted.");

    const storageId = await consumeEvidenceUploadIntent(
      ctx,
      upload.uploadIntentId,
      {
        householdId,
    ...
    const rawToken = "task16-private-view-token";

    const tokenHash = await hashEvidenceViewToken(rawToken);

    const viewTokenId = await ctx.db.insert("submissionEvidenceViewTokens", {
      tokenHash,
      ...
      viewerAuthUserId: "task16-parent",
    ...
    const authorized = await resolveEvidenceViewToken(ctx, tokenHash, now);

    assert(
      authorized?.storageId === args.storageId,
      "Authorized Parent could not resolve private evidence.",
    );

    await ctx.db.delete(membershipId);

    const afterRevocation = await resolveEvidenceViewToken(ctx, tokenHash, now);
    ...
    await ctx.db.delete(viewTokenId);

    await ctx.db.delete(submissionId);

    await ctx.db.delete(upload.uploadIntentId);
