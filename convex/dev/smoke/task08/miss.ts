...
export const run = internalMutation({
  ...
  handler: async (ctx): Promise<SmokeTestResult[]> => {
    ...
    const childId = await ctx.db.insert("children", {
      ...
      displayName: "TASK-08 Miss Child",
     ...
     */
    const generation = await generateOccurrencesForWindow(
      ctx,
      householdId,
      "2030-01-14",
      "2030-01-14",
    ...
    results.push(
      result(
        ...
        generation.createdCount === 1 &&
          generatedOccurrence?.state === "missed",
        generatedOccurrence?.state,
      ),
    );
     ...
     */
    const generatedLedger = await ctx.db
      .query("ledgerEntries")
      .withIndex("by_occurrence_kind", (q) =>
        q.eq("occurrenceId", generatedOccurrence!._id).eq("kind", "earning"),
      )
      .collect();
     ...
     * boundary cases.
     */
    const boundaryOccurrence = await ctx.db.insert("choreOccurrences", {
      householdId,
     ...
     */
    const atDeadline = await reconcileOccurrenceLifecycle(
      ctx,
      boundaryOccurrence,
      testNow,
    );

    const boundaryAtDeadline = await ctx.db.get(boundaryOccurrence);

    results.push(
      result(
        "Personal Chore is not missed exactly at deadline",
        atDeadline.changed === false &&
          boundaryAtDeadline?.state === "available",
        boundaryAtDeadline?.state,
      ),
    );
     ...
     * becomes missed.
     */
    const afterDeadline = await reconcileOccurrenceLifecycle(
      ctx,
      boundaryOccurrence,
      testNow + 1,
    );

    const boundaryAfter = await ctx.db.get(boundaryOccurrence);

    results.push(
      result(
        "Personal Chore becomes missed after deadline",
        afterDeadline.changed === true && boundaryAfter?.state === "missed",
        boundaryAfter?.state,
      ),
    );
     ...
     */
    const submittedOccurrence = await ctx.db.insert("choreOccurrences", {
      householdId,
      ...
      availabilityStartsAt: testNow - 2 * 60 * 60 * 1000,
    ...
    const submittedReconcile = await reconcileOccurrenceLifecycle(
      ctx,
      submittedOccurrence,
      testNow,
    );

    const submittedAfter = await ctx.db.get(submittedOccurrence);

    results.push(
      result(
        "Submitted Personal Chore is protected from deadline miss",
        submittedReconcile.changed === false &&
          submittedAfter?.state === "submitted",
        submittedAfter?.state,
      ),
    );
     ...
     * Personal occurrence.
     */
    const maintenanceOccurrence = await ctx.db.insert("choreOccurrences", {
      householdId,
      ...
      deadlineLocalTime: "11:00",

      deadlineDayOffset: 0,

      availabilityStartsAt: testNow - 2 * 60 * 60 * 1000,

      deadlineAt: testNow - 1,
    ...
    await ctx.db.patch(definitionId, {
      ...
      archivedByAuthUserId: "task08-smoke",

      updatedAt: testNow,
    });

    const maintenance = await runOccurrenceMaintenance(ctx, {
      now: testNow,
    ...
    const maintenanceAfter = await ctx.db.get(maintenanceOccurrence);

    results.push(
      result(
        "Maintenance reconciles overdue Personal Chore miss",
        maintenanceAfter?.state === "missed" &&
          maintenance.personalMissReconciledCount >= 1,
