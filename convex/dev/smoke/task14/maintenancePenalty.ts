...
export const run = internalMutation({
  ...
  handler: async (ctx) => {
    ...
    const householdId = await ctx.db.insert("households", {
      name: "TASK14 maintenance fixture",
    ...
    const childId = await ctx.db.insert("children", {
      ...
      displayName: "TASK14 Maintenance Child",
    ...
    const definitionId = await ctx.db.insert("choreDefinitions", {
      ...
      title: "TASK14 maintenance Claimable",
    ...
    try {
       ...
       * lifecycle.
       */
      const result = await runOccurrenceMaintenance(ctx, {
        now: deadlineAt + 1,
      ...
      assert(
        occurrence?.state === "failed" && claim?.state === "failed",
        "Maintenance must fail an overdue active Claim and occurrence.",
      );

      passed += 1;

      console.log("✅ 1/3 maintenance resolves overdue claimed lifecycle");

      assert(
        penalties.length === 1 &&
          penalties[0].childId === childId &&
          penalties[0].amountSek === -60,
        "Maintenance must create exactly one full-value Claim penalty.",
      );

      passed += 1;

      console.log("✅ 2/3 maintenance creates full-value penalty");
       ...
       */
      assert(
        result.claimableDeadlineReconciledCount >= 1,
       ...
       */
      await runOccurrenceMaintenance(ctx, {
        now: deadlineAt + 2,
      ...
      const penaltiesAfterRetry = await ctx.db
        .query("ledgerEntries")
        .withIndex("by_occurrence_kind", (q) =>
          q.eq("occurrenceId", occurrenceId).eq("kind", "penalty"),
        )
      ...
      assert(
        penaltiesAfterRetry.length === 1,
        "Repeated maintenance must not duplicate the penalty.",
      );

      passed += 1;

      console.log("✅ 3/3 repeated maintenance cannot double-charge");
