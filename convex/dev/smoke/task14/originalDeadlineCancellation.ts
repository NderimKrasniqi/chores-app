...
export const run = internalMutation({
  ...
  handler: async (ctx) => {
     ...
     * 17:00 UTC.
     */
    const deadlineAt = Date.UTC(2030, 0, 16, 17, 0, 0);

    const householdId = await ctx.db.insert("households", {
      name: "TASK14 cancellation boundary",
    ...
    const childId = await ctx.db.insert("children", {
      ...
      displayName: "TASK14 Cancellation Child",
    ...
    const definitionId = await ctx.db.insert("choreDefinitions", {
      ...
      title: "TASK14 cancellation fixture",
    ...
    async function createClaimedOccurrence(title: string) {
      ...
      const claim = await claimClaimableOccurrence(
        ctx,
        householdId,
        childId,
        occurrenceId,
        availabilityStartsAt + 60_000,
        false,
      );
    ...
    try {
       ...
       */
      const before = await createClaimedOccurrence("Cancel before deadline");
      ...
      const beforeOccurrence = await ctx.db.get(before.occurrenceId);

      const beforeLedger = await ledgerFor(before.occurrenceId);

      assert(
        beforeClaim?.state === "cancelled" &&
          beforeOccurrence?.state === "cancelled" &&
       ...
       */
      const exact = await createClaimedOccurrence("Cancel at exact deadline");
      ...
      console.log("✅ 2/4 exact-deadline Parent cancellation remains valid");
       ...
       */
      const overdue = await createClaimedOccurrence(
      ...
      const overdueBeforeReconcileClaim = await ctx.db.get(overdue.claimId);

      const overdueBeforeReconcileOccurrence = await ctx.db.get(
        overdue.occurrenceId,
      );

      assert(
        overdueBeforeReconcileClaim?.state === "claimed" &&
          overdueBeforeReconcileOccurrence?.state === "available",
      ...
      const overdueClaim = await ctx.db.get(overdue.claimId);

      const overdueOccurrence = await ctx.db.get(overdue.occurrenceId);

      const overdueLedger = await ledgerFor(overdue.occurrenceId);

      assert(
        overdueClaim?.state === "failed" &&
          overdueOccurrence?.state === "failed" &&
          overdueLedger.length === 1 &&
          overdueLedger[0].kind === "penalty" &&
          overdueLedger[0].amountSek === -100 &&
          overdueLedger[0].childId === childId,
